<script lang="ts">
  ...
  function getCachedPosts(handle: string): Post[] | null {
  ...
  function setCachedPosts(handle: string, posts: Post[]) {
    cache.set(handle, {
      data: posts,
      timestamp: Date.now()
    });
  ...
  async function getAuthorFeed(handle: string): Promise<Post[]> {
    // Check cache first
    const cachedPosts = getCachedPosts(handle);
    if (cachedPosts) {
      return cachedPosts;
    }
    ...
    try {
      ...
      setCachedPosts(handle, processedPosts);
...
<div class="container mx-auto py-8 max-w-2xl relative">
  ...
  <div class="h-[620px] overflow-y-scroll scrollbar">
    {#if posts.length > 0}
    <div class="space-y-6">
      {#each posts as post}
        <div class="cursor-pointer border rounded-lg p-4 transition-all text-lg bg-white dark:bg-zinc-900 hover:bg-neutral-50 dark:hover:bg-zinc-800 dark:text-white">
          ...
          <div class="border-t w-full pt-2.5 flex items-center gap-5 text-sm cursor-pointer"><div class="flex items-center gap-2 cursor-pointer"><img src="data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20fill='none'%20viewBox='0%200%2024%2024'%3e%3cpath%20fill='rgb(10,122,255)'%20d='M19.002%203a3%203%200%200%201%203%203v10a3%203%200%200%201-3%203H12.28l-4.762%202.858A1%201%200%200%201%206.002%2021v-2h-1a3%203%200%200%201-3-3V6a3%203%200%200%201%203-3h14Z'/%3e%3c/svg%3e" class="w-5 h-5"><p class="font-bold text-neutral-500 mb-px">Reply</p></div><div class="flex-1"></div><p class="cursor-pointer text-[#0A7AFF] font-bold hover:underline hidden min-[450px]:inline">View on Bluesky</p><p class="cursor-pointer text-[#0A7AFF] font-bold hover:underline min-[450px]:hidden"><span class="hidden min-[380px]:inline">View on </span>Bluesky</p></div>
        </div>
